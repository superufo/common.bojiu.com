...
func newEndpointCache(factory Factory, logger *zap.Logger, options endpointerOptions) *endpointCache {
	if logger == nil {
...
func (c *endpointCache) Update(event Event) {
	...
	c.logger.With(zap.Error(event.Err), zap.Stack("trace")).Info("show error")
...
func (c *endpointCache) updateCache(instances []string) {
	...
	for _, instance := range instances {
		...
		if err != nil {
			...
			c.logger.With(zap.Error(err), zap.Any("instance", instance), zap.Stack("trace")).Info("show error")
			continue
		}
